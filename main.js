...
setTimeout(() => {
  if (welcome) { welcome.classList.add('fade-out'); setTimeout(() => welcome.style.display='none',900); }
}, 2800);
...
document.getElementById('ipoForm').addEventListener('submit', async (e)=>{
  ...
  try{
    ...
    setProgress(100);
    setTimeout(()=>{ hideOverlay(); alert('✅ ¡Reporte enviado!'); form.reset(); },600);
  } catch(err){ console.error(err); hideOverlay(); alert('❌ Error al enviar el reporte.'); }
});
